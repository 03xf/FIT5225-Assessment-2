...
async function waitForQuery(queryId) {
  for (let attempt = 0; attempt < 180; attempt += 1) {
    const job = await api(`/api/search/by-file/${queryId}`);
    if (job.status === "READY") return job.matches || [];
    if (job.status === "FAILED") throw new Error(job.detail || "Temporary query failed.");
    setStatus(`Temporary query is processing (${job.status.toLowerCase()})...`);
    await new Promise((resolve) => setTimeout(resolve, 3000));
  }
  throw new Error("Temporary query is taking longer than expected. Please try again shortly.");
}
document.querySelector("#query-file-form").addEventListener("submit", async (event) => { event.preventDefault(); try { const file = document.querySelector("#query-file").files[0]; if (!file) throw new Error("Choose an image first"); const data = new FormData(); data.append("file", file); const job = await api("/api/search/by-file", {method:"POST",body:data}); setStatus("Temporary query queued. The page will show matches when processing completes."); const matches = await waitForQuery(job.query_id); render(matches); document.querySelector("#result-title").textContent = "Matches from temporary query"; setStatus("Temporary query processed; it was not archived."); } catch (error) { setStatus(error.message, true); } });
document.querySelector("#subscribe-form").addEventListener("submit", async (event) => { event.preventDefault(); try { const species = document.querySelector("#subscribe-species").value.trim(); const result = await api("/api/subscriptions", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({species})}); const prefix = result.status === "already-subscribed" ? `Already subscribed to ${species}.` : `Subscribed to ${species}.`; setStatus(result.email_confirmation_pending ? `${prefix} Confirm the SNS email to activate alerts.` : `${prefix} Email alerts are active for your subscribed species.`); event.target.reset(); } catch (error) { setStatus(error.message, true); } });
