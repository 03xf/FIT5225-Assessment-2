...
API Gateway (public static UI; JWT on /api/*) --> API Lambda
   ...
   Cognito user pool                              private S3 raw/thumbnail
                                  ...
                                  checksum-verified completion -> SQS -> dispatcher Lambda
                                                                          |
                                AWS workload identity federation         v
                                      +--------------------------> private GCP Cloud Run ML worker
                                                                            |--> private GCS model artifacts
                                                                            |
...
| GCP | Cloud Storage, Cloud Run, Secret Manager, Workload Identity Federation | ML container execution, private model artifacts and cross-cloud service identity |

## Critical implementation rules

1. The browser never receives AWS/GCP long-lived credentials or the Cloud Run worker URL.
