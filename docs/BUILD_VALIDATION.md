...
| Local code/IaC | `./scripts/preflight.ps1` | Python suite passes 16 tests; `terraform validate` succeeds. |

## Required next evidence

Build the Worker in a network-enabled environment, invoke `/healthz`, upload the
checksum-verified GCS model objects, then run `scripts/run_model_smoke.py` or a
Cloud Run request against a supplied image. Retain the output and Cloud Run logs
