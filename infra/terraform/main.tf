...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      PACIFICBIO_GCP_PROCESSOR_URL           = try(google_cloud_run_v2_service.worker[0].uri, "")
      PACIFICBIO_GCP_WIF_AUDIENCE            = try("//iam.googleapis.com/${google_iam_workload_identity_pool_provider.aws[0].name}", "")
      PACIFICBIO_GCP_WIF_SERVICE_ACCOUNT     = google_service_account.dispatcher_invoker.email
...
resource "google_storage_bucket" "replica" {
  name                        = "${var.project_name}-replica-${local.suffix}"
...
resource "google_artifact_registry_repository" "worker" {
  ...
  repository_id = "pacificbio-worker-${local.suffix}"
  description   = "Private Pacific BioArchive Cloud Run worker images"
  format        = "DOCKER"
}
resource "google_service_account" "worker" {
  account_id   = "pacificbio-worker-${local.suffix}"
  display_name = "Pacific BioArchive ML worker"
}
...
# The AWS dispatcher exchanges its Lambda role credentials for short-lived
# Google credentials. No GCP service-account private key is created.
resource "google_iam_workload_identity_pool" "aws" {
  count                     = var.aws_account_id == "" ? 0 : 1
  workload_identity_pool_id = "pacificbio-aws-${local.suffix}"
  display_name              = "Pacific BioArchive AWS dispatcher"
  description               = "Trusts only the dedicated AWS Lambda dispatcher role"
}
resource "google_iam_workload_identity_pool_provider" "aws" {
  count                              = var.aws_account_id == "" ? 0 : 1
  workload_identity_pool_id          = google_iam_workload_identity_pool.aws[0].workload_identity_pool_id
  workload_identity_pool_provider_id = "aws-dispatcher"
  display_name                       = "AWS Lambda dispatcher"
  attribute_mapping = {
    "google.subject"     = "assertion.arn"
    "attribute.account"  = "assertion.account"
    "attribute.aws_role" = "assertion.arn.extract('assumed-role/{role_name}/')"
  }
  attribute_condition = "attribute.account == '${var.aws_account_id}' && (attribute.aws_role == '${aws_iam_role.dispatcher.name}' || attribute.aws_role == '${aws_iam_role.api.name}')"
  aws { account_id = var.aws_account_id }
}
resource "google_service_account" "dispatcher_invoker" {
  account_id   = "pacificbio-dispatch-${local.suffix}"
  display_name = "AWS dispatcher Cloud Run invoker"
}
resource "google_service_account_iam_member" "dispatcher_wif" {
  count              = var.aws_account_id == "" ? 0 : 1
  service_account_id = google_service_account.dispatcher_invoker.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.aws[0].name}/attribute.aws_role/${aws_iam_role.dispatcher.name}"
}
resource "google_service_account_iam_member" "api_wif" {
  count              = var.aws_account_id == "" ? 0 : 1
  service_account_id = google_service_account.dispatcher_invoker.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.aws[0].name}/attribute.aws_role/${aws_iam_role.api.name}"
}
resource "google_secret_manager_secret" "callback_hmac" {
  secret_id = "pacificbio-callback-${local.suffix}"
  replication {
    auto {}
  }
}
resource "google_secret_manager_secret_version" "callback_hmac" {
  secret      = google_secret_manager_secret.callback_hmac.id
  secret_data = random_password.callback_hmac.result
}
resource "google_secret_manager_secret" "worker_key" {
  secret_id = "pacificbio-worker-key-${local.suffix}"
  replication {
    auto {}
  }
}
resource "google_secret_manager_secret_version" "worker_key" {
  secret      = google_secret_manager_secret.worker_key.id
  secret_data = random_password.worker_key.result
}
resource "google_secret_manager_secret_iam_member" "worker_callback" {
  secret_id = google_secret_manager_secret.callback_hmac.id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.worker.email}"
}
resource "google_secret_manager_secret_iam_member" "worker_key" {
  secret_id = google_secret_manager_secret.worker_key.id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.worker.email}"
}
resource "google_cloud_run_v2_service" "worker" {
  count    = var.worker_image_uri == "" ? 0 : 1
  name     = "${var.project_name}-worker-${local.suffix}"
  location = var.gcp_region
  template {
    ...
    containers {
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.callback_hmac.secret_id
            version = "latest"
          }
        }
      }
      env {
        name = "WORKER_SHARED_KEY"
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.worker_key.secret_id
            version = "latest"
          }
        }
      }
...
resource "google_cloud_run_v2_service_iam_member" "dispatcher_invoker" {
  count    = var.worker_image_uri == "" || var.aws_account_id == "" ? 0 : 1
  location = google_cloud_run_v2_service.worker[0].location
  name     = google_cloud_run_v2_service.worker[0].name
  role     = "roles/run.invoker"
  member   = "serviceAccount:${google_service_account.dispatcher_invoker.email}"
}
